namespace ServiceBrokerListener.UnitTests
{
	...
	public class SqlDependencyExTest
	{
		...
		{
			NotificationTypeTest(insertsCount, NotificationTypes.Insert);
			NotificationTypeTest(insertsCount, NotificationTypes.Delete);
			NotificationTypeTest(insertsCount, NotificationTypes.Update);
			NotificationTypeTest(
				insertsCount,
				NotificationTypes.Insert | NotificationTypes.Delete);
			NotificationTypeTest(
				insertsCount,
				NotificationTypes.Insert | NotificationTypes.Update);
			NotificationTypeTest(
				insertsCount,
				NotificationTypes.Delete | NotificationTypes.Update);
		}

		private static void NotificationTypeTest(int insertsCount, NotificationTypes testType)
		{
			...
			string connStr = TEST_CONNECTION_STRING)
		{
			...
			using (var sqlDependency = new SqlDependencyEx(
				new SqlDependencyExOptions
				{
					...
					TableName = TEST_TABLE_NAME,
					SchemaName = "temp"
				}, receiveDetails: false))
			{
				sqlDependency.TableChanged += (o, e) =>
				{
					Assert.AreEqual(NotificationTypes.None, e.NotificationType);
